...
# Sandbox MongoDB connection string for jobs database
SANDBOX_MONGO_URI = r"mongodb+srv://anujkamdar2006_db_user:UlaKG5HA8btbBmvm@sandboxportal.gsimmxy.mongodb.net/?retryWrites=true&w=majority"

def get_sandbox_job_by_id(job_id: str):
    """Fetch a single job from the sandbox database by ID"""
    from pymongo import MongoClient
    from bson import ObjectId
    try:
        client = MongoClient(SANDBOX_MONGO_URI)
        db = client["agno"]
        job = db.jobs.find_one({"_id": ObjectId(job_id)})
        client.close()
        return job
    except Exception as e:
        logger.error(f"Error fetching job {job_id} from sandbox: {e}")
        return None

def Ranking_agent(mdb_connection_string=None, student_artifact=None, embedder=None):
    # Always use sandbox connection for ranking (hackathon workaround)
    knowledge_base = Knowledge(
    vector_db=MongoVectorDb(
        collection_name="jobs",
        db_url=SANDBOX_MONGO_URI,
